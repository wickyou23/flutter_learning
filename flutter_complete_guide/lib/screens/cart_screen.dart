...
class _CartScreenState extends State<CartScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocListener<CartBloc, CartState>(
      listener: (_, state) {
        ...
        if (state is ValidateCartDoneState) {
          if (state.productsNameRemoved.isNotEmpty) {
            context
                .showAlert(
                    message:
                        '${state.productsNameRemoved.join(', ')}is not available.')
                .then((_) {
        ...
        body: BlocBuilder<CartBloc, CartState>(
          ...
          builder: (ctx, state) {
            return Container(
              ...
              child: Column(
                children: <Widget>[
                  Expanded(child: _cartItemList(state)),
                  Container(
                    height: 110,
  ...
  Widget _cartItemList(CartState state) {
  ...
  Widget _bottomWidget(CartState state) {
    CartReadyState stateReady;
    if (state is CartReadyState) {
      stateReady = state;
    }
      ...
      children: <Widget>[
          ...
          child: Container(
            child: Row(
              ...
              children: <Widget>[
                ...
                Text(
                  stateReady != null
                      ? '\$${stateReady.cart.getSumMoney().toStringAsFixed(2)}'
                      : '0.00',
                  style: context.theme.textTheme.title.copyWith(
                    fontSize: 20,
                  ),
        ...
        Container(
          ...
          child: RaisedButton(
            ...
            onPressed: stateReady != null
                ? () {
                    context.bloc<TransactionBloc>().add(
                          AddNewTransacionEvent(cart: stateReady.cart),
                        );
                  }
                : null,
          ),
